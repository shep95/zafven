...
| `/council <question> [constraints]` | **Superposition & collapse** — generates several diverse candidate answers, runs an adversarial oracle that eliminates the weak against your constraints, and ships the single survivor (a real best-of-N; slower but more reliable) |
...
| `/play <link/search>` · `/playnext` · `/loop track\|queue\|off` · `/queue` · `/shuffle` · `/remove` · `/clear` · `/nowplaying` · `/skip` · `/pause` · `/resume` · `/volume` · `/stop` | **Music** — joins your VC and streams audio from YouTube (via `yt-dlp` → FFmpeg) with a per-guild queue. `/playnext` jumps the line; `/loop` repeats a song or the whole queue (radio-style); the VC stays **music-only** (TTS won't talk over it) and she auto-leaves when the channel empties. On datacenter hosts YouTube may need a `MUSIC_COOKIE_FILE` (cookies.txt) to avoid its bot check. |
